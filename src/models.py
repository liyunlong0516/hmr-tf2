...
try:
    from tf_slim.python.slim.python.layers.initializers import variance_scaling_initializer
except ImportError:
    # Fallback for different tf_slim versions
    try:
        from tf_slim import variance_scaling_initializer
    except ImportError:
        # Last resort - use a simple initializer
...
def Encoder_resnet(x, is_training=True, weight_decay=0.001, reuse=False):
    ...
    variables = tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES, scope='resnet_v2_50')
...
def Encoder_fc3_dropout(x,
    ...
    with tf.compat.v1.variable_scope(name, reuse=reuse) as scope:
        ...
        small_xavier = variance_scaling_initializer(
            factor=.01, mode='FAN_AVG', uniform=True) if variance_scaling_initializer else tf.initializers.variance_scaling(factor=.01, mode='FAN_AVG', distribution='uniform')
