"""
Renders mesh using PyRender for visualization.
"""
...
class SMPLRenderer(object):
    def __init__(self,
        ...
        # Initialize pyrender renderer if available
        if USE_PYRENDER:
    ...
    def __call__(self,
        ...
        if USE_PYRENDER:
            # Use pyrender for rendering
            imtmp = self._render_pyrender(
                verts, w, h, cam,
                do_alpha=do_alpha, img=img,
                far=far, near=near, color_id=color_id)
        else:
            # Use opendr for rendering (legacy)
    ...
    def _render_pyrender(self, verts, w, h, cam, do_alpha=False, 
                         img=None, far=None, near=None, color_id=0):
        """Render using pyrender"""
        if color_id is None:
            color = np.array(colors['light_blue'])
        else:
            color_list = list(colors.values())
            color = np.array(color_list[color_id % len(color_list)])
        ...
        material = pyrender.MetallicRoughnessMaterial(
            baseColorFactor=np.concatenate([color, [1.0]]),
            alphaMode='BLEND' if do_alpha else 'OPAQUE'
        )
        ...
        camera = pyrender.IntrinsicsCamera(
            ...
            znear=near if near else 0.05, zfar=far if far else 100.0
        )
        scene.add(camera)
        ...
        # Render
        flags = pyrender.RenderFlags.RGBA if do_alpha else pyrender.RenderFlags.NONE
        color_img, depth_img = self.renderer.render(scene, flags=flags)
        ...
        if not do_alpha and color_img.shape[2] == 4:
            color_img = color_img[:, :, :3]
        
        # If background image is provided, blend
        if img is not None:
            if img.max() > 1:
            ...
            # Resize background image to match rendered image size
            if img_normalized.shape[:2] != color_img.shape[:2]:
                img_normalized = cv2.resize(img_normalized, (color_img.shape[1], color_img.shape[0]))
            
            if do_alpha and color_img.shape[2] == 4:
                # Use alpha channel for blending
                alpha = color_img[:, :, 3:4]
                # Ensure img_normalized has the same number of channels
                if img_normalized.ndim == 2:
                    img_normalized = np.stack([img_normalized] * 3, axis=-1)
                color_img = color_img[:, :, :3] * alpha + img_normalized * (1 - alpha)
            else:
                # Just overlay
                color_img = color_img
...
def draw_skeleton(input_image, joints, draw_edges=True, vis=None, radius=None):
    ...
    for child in range(len(parents)):
        ...
        if vis is not None and vis[child] == 0:
            continue
        if draw_edges:
            cv2.circle(image, (point[0], point[1]), radius, tuple(map(int, colors['white'])),
                       -1)
            cv2.circle(image, (point[0], point[1]), radius - 1,
                       tuple(map(int, colors[jcolors[child]])), -1)
        else:
...
def draw_text(input_image, content):
    ...
    for key in sorted(content.keys()):
        ...
        cv2.putText(image, text, (start_x, start_y), 0, 0.45, tuple(map(int, black)))
        start_y += margin
